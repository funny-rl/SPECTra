...
class RNNAgent(nn.Module):
    ...
    def init_hidden(self):
        ...
        return self.fc1.weight.new(self.batch_size_run, self.n_agents, self.args.rnn_hidden_dim).zero_()
    ...
    def forward(self, inputs, hidden_state):
        ...
        if len(input_shape) == 3:
            ...
            hidden_state = hidden_state.reshape(input_shape[0], -1, self.args.rnn_hidden_dim)
            gru_out, _ = self.rnn(x, hidden_state)
            
            hidden_state = hidden_state.reshape(-1, self.n_agents, self.args.rnn_hidden_dim)
