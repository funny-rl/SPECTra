...
class Hypernetwork(nn.Module):
    def __init__(self, args, input_shape, last_action = False):
        ...
        self.ball_embedding = nn.Linear(self.input_shape[2], self.hypernet_embed)
        ...
        if last_action: # Not work in grf
...
class SSMixer(nn.Module):
    def __init__(self, args, abs = True):
        ...
        self.state_component = [4 * self.n_agents, 4* self.n_enemies, 6]
    ...
    def _get_input_shape(self, state_component):
        entity_type = [self.n_agents, self.n_enemies, 1] # U have to change this when u change your state entity sequence
        state_shape = []
        state_feature_dims = []
